...
My reported model accuracy is **77.7%** - essentially the baseline a dummy classifier would achieve on this cohort. This is why accuracy was treated as a **secondary/decorative metric** and all real decisions were made on ROC-AUC, KS, and Brier.

### 1.3 What Data I Used

I loaded only **origination-time columns** - information known at the moment a loan application is submitted:
...
Before any model was trained, I ranked all features by **Mutual Information (MI)** - a measure of how much knowing a feature's value reduces uncertainty about whether a loan will default.
...
I benchmarked 13 model families under identical conditions on 20,000 training samples to select which families to take to the final tuning stage.
...
My headline Brier score of **0.148** sits under the OOT base-rate baseline (~0.16), confirming the calibrated probabilities are trustworthy inputs to the profit layer even under out-of-time drift.
...
0.20–0.40 → Acceptable ✓ (I am here)
0.40–0.60 → Good
...
On an imbalanced dataset, ROC-AUC can be misleadingly optimistic because it gives equal weight to the majority (good) class. PR-AUC focuses on minority class performance. A random classifier scores PR-AUC ≈ **0.20** (the OOT base rate). My **0.371** is **~1.9× better than random** - meaningful signal for catching defaults.
...
My model edges under the naive baseline, confirming the PD scores carry real information beyond the base rate even under OOT drift.
...
I tested every possible threshold from 0 to 1 (in 101 steps):
...
For XGBoost, I use **TreeExplainer** - an exact algorithm that computes Shapley values in polynomial time without approximation.
...
| Limitation | Detail |
|---|---|
| **No bureau data** | I have no FICO scores, no payment history outside LC - bureau models typically score 0.75+ AUC vs my ~0.70 OOT |
